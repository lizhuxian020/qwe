...
#import "CDSPageViewController.h"
#import "CDSProgressView.h"
#import "CDSPageConsts.h"
#import "UIView+CDSPageFrame.h"
...
@interface CDSPageViewController ()
{
    UIColor *_norColor;
    UIColor *_selColor;
...
/** 滚动条 */
@property (nonatomic, strong) CDSProgressView *pregressView;
...
@implementation CDSPageViewController
...
- (UIScrollView *)titleScrollView
{
    if (!_titleScrollView) {
        ...
        UIView *line = [UIView drawLineView:0];
...
- (void)viewDidLayoutSubviews
{
    ...
    self.titleScrollView.frame = CGRectMake(0, 0, ScreenW, tH);
    self.contentScrollView.frame = CGRectMake(0, tH, ScreenW, ScreenH - (tY + tH + bD));
...
- (void)setUpAllTitle
{
    ...
    if (_isShowPregressView) { //如果没子控制器的时候不加载
        //指示条
        _pregressView = [CDSProgressView new];
...
- (int)getCurrentSelectedIndex {
    NSInteger tagI = _contentScrollView.contentOffset.x / ScreenW;
    return (int)tagI;
}

- (UIViewController *)getCurrentVC {
    int idx = self.getCurrentSelectedIndex;
    if (idx > self.childViewControllers.count) {

        KMTLogError(@"=====ERROR>>>>>订单管理, 当前索引超出范围%d", idx);
