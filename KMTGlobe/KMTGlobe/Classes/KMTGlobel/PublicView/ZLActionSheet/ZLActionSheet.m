...
@interface ZLActionSheet()<ZLSheetViewDelegate>

@property (strong, nonatomic) UIButton *bgButton;
@property (strong, nonatomic) UIView *contentView;
@property (strong, nonatomic) ZLSheetView *sheetView;
@property (strong, nonatomic) ZLSheetHead *titleView;
@property (strong, nonatomic) ZLSheetFoot *footView;
...
@property (assign, nonatomic) ZLSheetStyle sheetStyle;
@end
               ...
               superView:(UIView *)superView
{
    if (self = [super initWithFrame:[[UIScreen mainScreen] bounds]]) {
        ...
        //取消按钮View
        self.footView = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([ZLSheetFoot class]) owner:self options:nil].lastObject;
        [self.footView.footButton.titleLabel setTextColor:[UIColor blueColor]];
        [self addSubview:self.footView];
        //选择TableView
        self.sheetView = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([ZLSheetView class]) owner:self options:nil].lastObject;
...
///初始化默认样式
- (id)upDefaultStyeWithItems:(NSArray *)itemTitles title:(NSString *)title selfView:(ZLActionSheet *)selfView{
    //半透明背景按钮
    ...
    BOOL isTitle = NO;
    if (title.length > 0) {
        selfView.titleView = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([ZLSheetHead class]) owner:selfView options:nil].lastObject;
    ...
    selfView.footViewY = kWH - kCellH - kMargin - UI_NAVIGATION_BAR_STATUS_BAR_HEIGHT;
    selfView.footView.frame = CGRectMake(kMargin, kWH + selfView.contentVH + kMargin, kMW, kCellH);
    selfView.contentViewY = kWH - CGRectGetHeight(selfView.footView.frame) - selfView.contentVH - kMargin * 2 - UI_NAVIGATION_BAR_STATUS_BAR_HEIGHT;
...
///初始化微信样式
- (id)upWeiChatStyeWithItems:(NSArray *)itemTitles title:(NSString *)title selfView:(ZLActionSheet *)selfView
{
    ...
    selfView.bgButton.frame = CGRectMake(0, 0, kWW, kWH);
    selfView.footView.footButton.titleLabel.font = [UIFont systemFontOfSize:16];
    ...
    selfView.footViewY = kWH - kCellH - UI_NAVIGATION_BAR_STATUS_BAR_HEIGHT;
    selfView.footView.frame = CGRectMake(0, selfView.footViewY + selfView.contentVH, kWW, kCellH);
    
    selfView.contentViewY = kWH - CGRectGetHeight(selfView.footView.frame) - selfView.contentVH - kMargin - UI_NAVIGATION_BAR_STATUS_BAR_HEIGHT;
    ...
    [selfView.footView.footButton setTitleColor:kCellTextColor forState:UIControlStateNormal];
    return selfView;
}

///初始化TableView样式
- (id)upTableStyeWithItems:(NSArray *)itemTitles title:(NSString *)title selfView:(ZLActionSheet *)selfView
{
    ...
    if (title.length > 0) {
        selfView.titleView = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([ZLSheetHead class]) owner:selfView options:nil].lastObject;
        selfView.titleView.headLabel.text = title;
        selfView.titleView.headLabel.textAlignment = NSTextAlignmentCenter;
        isTitle = YES;
        [selfView.contentView addSubview:selfView.titleView];
    }
    selfView.sheetView.cellTextColor = [UIColor blackColor];
    ...
    selfView.contentViewY = kWH - selfView.contentVH - UI_NAVIGATION_BAR_STATUS_BAR_HEIGHT;
    selfView.contentView.frame = CGRectMake(0, kWH, kWW, selfView.contentVH);
...
- (void)show
{
    if (_sheetStyle == ZLSheetStyleDefault) {
        [self pushDefaultStyeSheetView];
    }
    else if (_sheetStyle == ZLSheetStyleWeiChat) {
        [self pushWeiChatStyeSheetView];
    }
    else if (_sheetStyle == ZLSheetStyleTable) {
        [self pushTableStyeSheetView];
    }
}
