...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
      ...
      //  _borderColor = UIColorFromRGB(0x7f7f7f);
        ...
        _normalBackgroundImage = [self imageWithColor:YMRGBValue(0xFFECC2) size:CGSizeMake(1.0, 1.0)];
...
- (void)setTagAry:(NSArray *)tagAry delegate:(id)delegate {
    if (delegate) _tagDelegate = delegate;
...
- (void)disposeTags:(NSArray *)ary {
    ...
    for (NSString *tagTitle in ary) {
        ...
        //计算每个tag的宽度
        CGSize contentSize = [tagTitle fdd_sizeWithFont:[UIFont systemFontOfSize:15] constrainedToSize:CGSizeMake(self.frame.size.width-_tagOriginX*2, MAXFLOAT)];
...
-(void)setTagAry:(NSArray *)tagAry Block:(void (^)(NSInteger, NSString *, UIButton *))ckickedBlock{
    [self setTagAry:tagAry delegate:nil];
    _tagClickedBlock = ckickedBlock;
}
...
- (void)buttonAction:(UIButton *)sender {
    ...
    if (_tagClickedBlock) {
        _tagClickedBlock(sender.tag,sender.titleLabel.text,sender);
    }
}
