...
- (void)viewDidLoad {
    ...
    UIView *hView = [UIView initWithFrame:Rect(0, 0, ScreenWidth, 100) backGroundColor:WHITECOLOR];
        ...
        dispatch_async(serialQueue, ^{
            [self processData:^(id success) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [kself.rootTableView reloadData];
                    self.locationManager = [[JFLocation alloc] init];
                    kself.locationManager.delegate = self;
                });
            }];
        });
    }
...
- (JFCityHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[JFCityHeaderView alloc] initWithFrame:CGRectMake(0, 22, self.view.bounds.size.width, 80)];
...
- (JFSearchView *)searchView {
    if (!_searchView) {
        ...
        _searchView.backgroundColor = RGBA(0, 0, 0, 0.2);
...
- (UITableView *)rootTableView {
    if (!_rootTableView) {
        ...
        _rootTableView.contentInset = UIEdgeInsetsMake(100, 0, 0, 0);
    }
...
-(void)headCancelBtnClicked:(id)sender{
    [self dismissViewControllerAnimated:YES completion:nil];
}
