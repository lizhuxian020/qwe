...
@property (nonatomic,assign)int Month_from;
...
// 获取月列表
- (NSArray<NSString *> *)fetchMonthsWithYear:(int)year andFromMonth:(int)fromM
{
    BOOL isCan = _month_year == year && _Month_from == fromM && _months && [_months count] > 0;
    if (!isCan) {
        ...
        NSMutableArray *tempAry = [NSMutableArray array];
        for (int i = fromM; i <= length; i++) {
            [tempAry addObject:[NSString stringWithFormat:@"%d",i]];
        }
        _months = tempAry;_month_year= year;_Month_from = fromM;
    }
    return _months;
}
// 获取日列表(不包含星期几)
- (NSArray<NSString *> *)fetchDaysWithYear:(int)year month:(int)month fromD:(int)fromD
{
...
// 获取小时
- (NSArray<NSString *> *)fetchHourFromH:(int)fromH
{
    BOOL isCan = _hours && [_hours count] > 0;
    if (!isCan) {
        NSMutableArray *tempAry = [NSMutableArray array];
        for (int i = fromH; i <= 23; i++) {
...
// 获取分钟
- (NSArray<NSString *> *)fetchMinutesFromM:(int)fromM
{
    BOOL isCan = _minutes && [_minutes count] > 0;
    if (!isCan) {
        NSMutableArray *tempAry = [NSMutableArray array];
        for (int i = fromM; i <= 59; i++) {
            NSString *str = [NSString stringWithFormat:@"%d",i];
            if (i < 10) {
                str = [NSString stringWithFormat:@"0%@",str];
            }
