...
#import "UIImage+KMTUtils.h"

@implementation UIImage (KMTUtils)
...
+(UIImageView *)createRowPhotoImageWithOrginX:(CGFloat)x andOrginY:(CGFloat)y andImage:(UIImage *)imageName{
    UIImage *imagep = imageName;
    UIImageView *imageView = [[UIImageView alloc]initWithFrame:Rect(x, y, imagep.size.width, imagep.size.height)];
...
- (void)imageWithCorner:(CGSize)size fillColor:(UIColor *)fillColor completeBlk:(void(^)(UIImage *))complete {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        CGRect rect = Rect(0, 0, size.width, size.height);
